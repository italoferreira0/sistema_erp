...
class EmployeesPermission(permissions.BasePermission):
    message = 'Não tem essa permissão para gerenciar.'

    def has_permission(self, request, _view):
        return check_permission(request.user, request.method, permission_to='employee')

class GroupPermission(permissions.BasePermission):
    message = 'O funcionario não tem essa permissão para gerenciar os grupos.'
...
class GroupPermissionsPermission(permissions.BasePermission):
    message = 'O funcionario não tem permissão para gerenciar as permissões.'

    def has_permission(self, request, _view):
        return check_permission(request.user, request.method, permission_to='permission')

class TaskPermission(permissions.BasePermission):
    message = 'O funcionario não tem permissão para gerenciar as tarefas de todos os funcionarios.'
