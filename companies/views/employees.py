...
from companies.serializer import EmployeeSerializer, EmployeesSerializer
...
class EmployeeDatail(Base):
    ...
    def put(self, request, employee_id):
        ...
        if email != employee.user.email and User.objects.filter(email=email).exists():
            raise APIException('Esse email já está em uso', code='email_already_user')
    ...
    def delete(self, request, employee_id):
        ...
        if check_if_owner:
            raise APIException('Você demitir o dono da empresa.')
